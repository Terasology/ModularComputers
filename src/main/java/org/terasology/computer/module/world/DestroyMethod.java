...
public class DestroyMethod extends AbstractModuleMethodExecutable<Object> {
    ...
    public DestroyMethod(String methodName, WorldProvider worldProvider, BlockEntityRegistry blockEntityRegistry,
        ...
        super("Destroys the block in the specified direction. The resulting items from destroying the " +
                "block are scattered on the ground.", "Boolean", "Whether destroying the specified block was " +
                "successful.");
    ...
    public Object onFunctionEnd(int line, ComputerCallback computer, Map<String, Variable> parameters,
        ...
        if (blockBeforeDestroy != replaceBlock) {
            ...
            harvestedEntity.send(new DestroyEvent(computer.getComputerEntity(), EntityRef.NULL, Assets.getPrefab(
                    "ModularComputers:harvestDamage").get()));
