...
public class StorageModuleCommonSystem extends BaseComponentSystem {
    ...
    public void preBegin() {
        if (moduleConfigManager.getBooleanVariable("ModularComputers", "registerModule.storage", true)) {
            String inventoryModulePageId = DocumentationBuilder.getComputerModulePageId(
                    InventoryModuleCommonSystem.COMPUTER_INVENTORY_MODULE_TYPE);
            String inventoryModuleDumpMethodPageId = DocumentationBuilder.getComputerModuleMethodPageId(
                    InventoryModuleCommonSystem.COMPUTER_INVENTORY_MODULE_TYPE, "dump");
            ...
            computerModuleRegistry.registerComputerModule(
                    ...
                    "This module allows storing items within the computer itself." +
                            "Only one module of this type can be installed in a computer " +
                            "at a time. Player does not have access to the storage itself via user interface, however " +
                            "<h navigate:" + inventoryModulePageId + ">Inventory manipulator</h> module can be used to access it " +
                            "and store in an external " +
