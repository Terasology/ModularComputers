...
public class PublicCommunicationChannelCustomObject implements CustomObject, CommunicationChannel {
    ...
    public void sendMessage(String message, ComputerCallback computer, long expireOn) {
        communicationChannels.addPublicMessage(channelName, new Vector3i(computer.getComputerLocation(),
                RoundingMode.FLOOR), range, message, expireOn);
    }

    @Override
    public Map<String, Variable> consumeMessage(long currentTime, ComputerCallback computer) {
        return communicationChannels.consumeNextPublicMessage(currentTime, channelName, new Vector3i(computer.getComputerLocation(),
                RoundingMode.FLOOR), range);
    }
